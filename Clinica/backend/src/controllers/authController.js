...
const register = asyncHandler(async (req, res) => {
  ...
  res.status(201).json({ success: true, data: usuario.toSafeJSON() });
});

const login = asyncHandler(async (req, res) => {
  const { email, password } = req.body;
  const usuario = await Usuario.scope('withPassword').findOne({ where: { email } });
  if (!usuario || !(await usuario.validatePassword(password))) throw new AppError('Credenciales inválidas', 401);
  const tokens = await authService.issueTokens(usuario, { userAgent: req.get('user-agent'), ip: req.ip });
  res.json({ success: true, data: { usuario: usuario.toSafeJSON(), ...tokens } });
});
